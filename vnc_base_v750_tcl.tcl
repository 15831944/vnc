...
set PB_version 7.5.0
if { [info exists mom_sim_post_builder_version] } {
 set __sim_PB_version  [join [split $mom_sim_post_builder_version .] ""]
 } else {
 set __sim_PB_version  0
}
set __vnc_base_version [join [split $PB_version .] ""]
if { $__vnc_base_version > $__sim_PB_version } {
 set __msg "ERROR :\n\
 \nThe VNC needs to be created with Post Builder v$PB_version or newer\
 \nin order to work with \"vnc_base_v${__vnc_base_version}_tcl.txt\"!"
 catch { SIM_mtd_reset }
 MOM_abort $__msg
}
...
proc VNC_sim_nc_block { block_buffer } {
  ...
  global mom_sim_nc_block_buffer mom_sim_nc_block
  set mom_sim_nc_block_buffer $o_buff
  ...
  if $vnc_msg {
   ...
   set mom_sim_o_buffer [string trim $o_buff]
   ...
   if { [info exists mom_sim_other_nc_codes] } {
    foreach code $mom_sim_other_nc_codes {
     VNC_parse_nc_word mom_sim_o_buffer $code 1 $cb_cmd
    }
   }
   if { [info exists mom_sim_other_nc_codes_ex] } {
    foreach code $mom_sim_other_nc_codes_ex {
     VNC_parse_nc_word mom_sim_o_buffer $code 2 $cb_cmd
    }
   }
   ...
   if { ![string match "" $mom_sim_prog_rewind_stop_code] } {
    VNC_parse_nc_block o_buff $mom_sim_prog_rewind_stop_code VNC_rewind_stop_code
   }
  } ;# Regular block
 }
  ...
  foreach addr $addr_list {
   ...
   if { [info exists mom_sim_address($addr,leader)] } {
    ...
    if { [eval $cmd] == 1 } {
     ...
     if { ![string compare "I" $addr] ||\
      ![string compare "J" $addr] ||\
      ![string compare "K" $addr] ||\
      ![string compare "R" $addr] } {
     ...
     set local_buff "$leader[string trim [string range $local_buff $idx end]]"
...
proc VNC_cook_F_val { simulate_block args } {
  ...
  if { ![string compare "MILL" $mom_sim_nc_register(MACHINE_MODE)] } {
...
proc VNC_format_val { fmt val args } {
  ...
  if { $mom_sim_format($fmt,lead_zero)  == 0  && \
   ...
   if { ![catch { set val [expr double($tmp_val)] }] } {
    return $val
    } else {
    return $tmp_val
   }
  ...
  if { [info exists mom_sim_advanced_kinematic_jct] } {
   ...
   if { [info exists mom_sim_reverse_4th_table] && $mom_sim_reverse_4th_table } {
    ...
    if { [info exists mom_kin_4th_axis_type]  &&  \
     ![string compare "TABLE" [string toupper $mom_kin_4th_axis_type]] } {
     if { ![string compare "standard" $mom_kin_4th_axis_rotation] } {
   ...
   if { [info exists mom_sim_reverse_5th_table] && $mom_sim_reverse_5th_table } {
    ...
    if { [info exists mom_kin_5th_axis_type]  &&  \
     ![string compare "TABLE" [string toupper $mom_kin_5th_axis_type]] } {
     if { ![string compare "standard" $mom_kin_5th_axis_rotation] } {
...
proc VNC_CalculateDurationTime { linear_or_angular delta } {
  ...
  if { ![string compare "RAPID" $sim_feedrate_mode] } {
...
proc VNC_set_feedrate_mode { cutting_mode } {
  ...
  if { ![string compare "RAPID" $cutting_mode] } {
...
proc VNC_unmount_tool { tool_ug_name } {
  ...
  if { [string compare "undefined" $tool_ug_name] } {
...
#=======================================================================
proc VNC_rewind_stop_code {} {
  PB_SIM_call PB_CMD_vnc__rewind_stop_code
 }
...
proc VNC_circular_move { direction args } {
  ...
  if { ![string compare "CLW" $direction] } {
...
proc VNC_set_ref_jct { sim_tool_name } {
  ...
  if { [string length $sim_tool_name] && [string compare "undefined" $sim_tool_name] }\
  {
...
proc VNC_move_linear_zcs { mode args } {
  if { [string compare "CURRENT" $mode] } {
  ...
  if { [info exists mom_sim_reverse_4th_table]  && \
   ...
   if { [info exists mom_kin_4th_axis_type]  &&  ![string compare "TABLE" [string toupper $mom_kin_4th_axis_type]] } {
    if { ![string compare "standard" $mom_kin_4th_axis_rotation] } {
  ...
  if { [info exists mom_sim_reverse_5th_table]  && \
   ...
   if { [info exists mom_kin_5th_axis_type]  &&  ![string compare "TABLE" [string toupper $mom_kin_5th_axis_type]] } {
    if { ![string compare "standard" $mom_kin_5th_axis_rotation] } {
...
proc VNC_trace { args } {
  ...
  for { set i $start_idx } { $i < $level } { incr i } {
   if { [string compare "PB_SIM_call" [lindex [info level $i] 0]] } {
