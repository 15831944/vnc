...
proc VNC_sim_nc_block { block_buffer } {
  ...
  PB_SIM_output_qc_cmd_string "# --- $o_buff"
  if 0 {
  ...
  if $vnc_msg {
   ...
   if [llength [info commands "PB_CMD_vnc__preprocess_nc_block"]] {
    ...
    set mom_sim_o_buffer $o_buff
    } elseif [llength [info commands "PB_CMD_vnc__process_nc_block"]] {
    set o_buff [PB_SIM_call PB_CMD_vnc__process_nc_block]
    if [string match "" [string trim $o_buff]] {
     return
    }
    set mom_sim_o_buffer $o_buff
    } elseif [llength [info commands "PB_CMD_vnc__condition_nc_block"]] {
    set o_buff [PB_SIM_call PB_CMD_vnc__condition_nc_block]
    if [string match "" [string trim $o_buff]] {
     return
    }
    set mom_sim_o_buffer $o_buff
   ...
   VNC_cook_F_val $simulate_block
   PB_SIM_call PB_CMD_vnc__G_spin_code
       ...
       if [info exists mom_sim_address($addr,format)] {
        ...
        if 0 {
         ...
         if { $mom_sim_format($fmt,lead_zero) == 0  && \
          ...
          PB_VNC_output_debug_msg "Unrecognized data : $o_buff"
          } else {
          ...
          if [string match "" $tmp_val] {
           ...
           if [catch { set val [expr $tmp_val / pow(10,$n_frac)] } ] {
            PB_VNC_output_debug_msg "Error in data : ($val) ($tmp_val) ($o_buff)"
            return 0
           }
          }
         }
        }
        if [catch { set val [VNC_format_val $fmt $val] } ] {
         PB_VNC_output_debug_msg "Error in data : ($val) ($tmp_val) ($o_buff)"
         return 0
        }
       } ;# Valid format
...
proc VNC_cook_F_val { simulate_block args } {
  ...
  if [info exists mom_sim_nc_register(F)] {
   ...
   set fmt ""
   if { [info exists mom_sim_feed_param] &&\
    [info exists mom_sim_rapid_feed_mode] &&\
    ...
    if [expr $simulate_block < 1000] {                          ;# Pure rotary move
     ...
     } elseif [expr fmod($simulate_block,1000) == 0] {           ;# Pure linear move
     if [string match "RAPID" $mom_sim_nc_register(MOTION)] {
   ...
   if [catch { set val [VNC_format_val $fmt $mom_sim_nc_register(F)] } ] {
...
#=======================================================================
proc  MTX3_xform_csys { xa yb zc CSYS } {
  upvar $CSYS csys
  set xa [expr -1*$xa]
  set yb [expr -1*$yb]
  set zc [expr -1*$zc]
